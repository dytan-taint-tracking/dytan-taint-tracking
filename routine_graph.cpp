#include "routine_graph.h"

RoutineGraph::RoutineGraph(RTN rtn) {
	resultsFile = fopen("/dev/null", "w+");
	debugFile = fopen("/dev/null", "w+");
	initializeRoutineInfo(rtn);
}

RoutineGraph::RoutineGraph(RTN rtn, FILE * specifiedResults, FILE * specifiedDebug) {
...
void RoutineGraph::initializeRoutineInfo(RTN rtn) {
	...
	{
		/*	this is the first pass over all the instructions
...
void RoutineGraph::mapper(INS ins, RTN rtn) {
...
//	ADDRINT exit = 3;
	...
	if(INS_IsBranch(ins)) {
		...
		fprintf(debugFile, "found a Branch pointing to : %#x", (uint) target);
...
void RoutineGraph::BlockBuilder(RTN rtn) {
...
if (currentBlock != NULL) {
	fprintf(debugFile, "GOING TO SET THE ENDING TO: %#x", (uint) INS_Address(previous));
	currentBlock->setEnding(INS_Address(previous));
}
...
void RoutineGraph::BranchIns(INS currentIns, BasicBlock * currentBlock) {
...
BasicBlock * RoutineGraph::Get_BasicBlock_At(ADDRINT targetAddress ) {
...
void RoutineGraph::buildPostDominanceTree(BasicBlock * currentBlock, queue<BasicBlock *> blockQueue) {
...
void RoutineGraph::buildDominanceTree(BasicBlock * currentBlock, queue<BasicBlock *> blockQueue) {
